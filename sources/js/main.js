...
//func for clicking on Add button
document.getElementById('add').addEventListener('click', function() {
var value = document.getElementById('task').value;
if (value) {
//  console.log("entry");
  addItemtasks(value);
 //resets the value in input text box
  document.getElementById('task').value = '';

  }
});

//Adding item to the task_lists

function addItemtasks(job){
  var lists = document.getElementById('task_lists');
  ...
  deletion.innerHTML = deleteSVG;

  var finish = document.createElement(button);
  finish.classList.add('finish');
  finish.innerHTML = finishSVG;

  buttons.appendChild(deletion);
  buttons.appendChild(finish);
  item.appendChild(buttons);
  // inserts the recent task at top
